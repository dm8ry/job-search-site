<?php session_start(); 
	...
	if ( isset($_POST['the_curr_page']) && isset($_POST['recs_per_page']) && isset($_SESSION['princ_email']) )
	{
		...
		$part_2 = '';
					  ...
					  $part_2 = $part_2. 	'<tr>' ."\n" .
											'<td>'.$arr_recent_activities[$idx]['id'].'</td>' ."\n" .
											'<td>'.$arr_recent_activities[$idx]['the_dt'].'</td>' ."\n" .   
											'<td>'.$arr_recent_activities[$idx]['name_eng'].'</td>' ."\n" .
											'<td><button type="button" class="btn btn-default" onclick="getRecentActivityDetails('.$arr_recent_activities[$idx]['id'].')" >Details...</button></td>' ."\n" .
											'</tr>' ."\n" ;
